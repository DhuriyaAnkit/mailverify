...
export const SignUp: React.FC = () => {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github') => {
    try {
      ...
      showToast(err.message || `Could not sign up with ${provider}`, 'error');
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      showToast('Account created successfully! Check your inbox if verification is required.', 'success');
      setMessage({
        type: 'success',
        text: 'Account created successfully! If email verification is enabled, check your inbox to verify.'
      });
      setName('');
      setEmail('');
      setPassword('');
    ...
    <div className="signup-page-container" style={{
      ...
      >
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {/* Terms checkbox */}
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', textAlign: 'left', marginTop: '4px' }}>
            <input
              type="checkbox"
              id="agree-checkbox"
              required
            ...
            />
            <label htmlFor="agree-checkbox" style={{ fontSize: '12.5px', color: 'var(--text-secondary)', lineHeight: 1.4, cursor: 'pointer' }}>
              I agree to the <a href="#terms" style={{ color: 'var(--accent-color)', textDecoration: 'none', fontWeight: 500 }}>Terms of Service</a> and <a href="#privacy" style={{ color: 'var(--accent-color)', textDecoration: 'none', fontWeight: 500 }}>Privacy Policy</a>.
            </label>
          </div>
